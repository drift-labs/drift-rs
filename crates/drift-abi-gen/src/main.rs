...
fn generate_abi_types(idl: &Idl) -> String {
    ...
    for account in &idl.accounts {
        ...
        let discriminator: TokenStream = format!("{:?}", sighash(&idl.name, &account.name))
            .parse()
            .unwrap();
    ...
    for instr in &idl.instructions {
        ...
        let discriminator: TokenStream =
            format!("{:?}", sighash(&idl.name, &name)).parse().unwrap();
        let struct_def = quote! {
...
/// Output is emitted to `NAME_VERSION_abi.rs`
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rust_abi_types = format_rust_code(&generate_abi_types(&idl));

    // Write the generated Rust code to the file
    let file_name = format!("{}_abi.rs", idl.name);
    let mut file = File::create(&file_name)?;
    file.write_all(rust_abi_types.as_bytes())?;
