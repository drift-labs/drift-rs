...
use drift::{controller::position::PositionDirection, math::auction, state::user::{Order, OrderType}};
...
pub fn get_auction_price(order: &Order, slot: u64, price: i64) -> BigInt {
    if is_one_of_variant(&order.order_type, &[OrderType::Market, OrderType::TriggerMarket, OrderType::Limit, OrderType::TriggerLimit]) {
...
fn get_auction_price_for_fixed_auction(order: &Order, slot: u64) -> BigInt {
    ...
    match order.direction {
        PositionDirection::Long => {
            let price_delta = auction_end_price.clone() - auction_start_price.clone() * delta_numerator / delta_denominator;
            auction_start_price.clone() + price_delta
        }
        PositionDirection::Short => {
            let price_delta = auction_start_price.clone() - auction_end_price.clone() * delta_numerator / delta_denominator;
            auction_start_price.clone() - price_delta
        }
    }
}

fn get_auction_price_for_oracle_offset_auction(order: &Order, slot: u64, oracle_price: i64) -> BigInt {
    let slots_elapsed = slot - order.slot;
